...
interface ImageCardProps {
	favoriteImage: GalleryImage;
	favoriteImages: GalleryImage[];
}

export function ImageCard({ favoriteImage,favoriteImages,setFavoriteImages }: ImageCardProps) {
	const [isFavorited,setIsFavorited]=useState(true);


	function handleFavoriteClick(e: React.MouseEvent) {
		e.stopPropagation();
		setFavoriteImages(prev => [...(prev.map(i => i.image_id!==favoriteImage.image_id))]);
	}
		...
		<Link to={`/images/${image.image_id}`}>
			<article className="group relative aspect-[4/3] overflow-hidden rounded-2xl border border-border shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
				...
				>
					<Heart
						size={18}
						className={`transition-all duration-300 fill-accent text-accent scale-110`}
					/>
				</button>
